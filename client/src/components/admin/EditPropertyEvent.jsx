...
const EditPropertyEvent = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        if (!token) { setError("Authentication missing. Please log in again."); setLoading(false); return; }
        const response = await axios.get(`${API_BASE_URL}/api/admin/events/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/api/admin/events/${id}`, data, {
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "multipart/form-data" },
      });
      setSuccess("Event updated successfully!");
      setTimeout(() => navigate("/admin-dashboard/manage-events"), 1500);
    } catch (err) {
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm min-h-[400px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-2 text-slate-400">
          ...
          <span className="text-xs font-semibold">Loading event data…</span>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col" style={{ fontFamily: '"Inter", sans-serif' }}>
      ...
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <Link to="/admin-dashboard/manage-events" className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-50 border border-slate-200 text-slate-500 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-600 transition-all">
          ...
          <div>
            <h2 className="text-base font-extrabold text-slate-900 tracking-tight leading-none">Edit Event</h2>
            <p className="text-[10px] text-slate-400 font-medium mt-0.5">Event Management System</p>
          </div>
        </div>
        <span className="bg-amber-50 text-amber-600 text-[10px] font-bold px-2.5 py-1 rounded-full border border-amber-100 flex items-center gap-1">
          <Home className="w-3 h-3" /> Editor Mode
        </span>
      </div>
      ...
      <div className="p-5 max-w-5xl mx-auto w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* ── Basic Info ── */}
          <Section icon={<PencilLine className="w-3.5 h-3.5 text-sky-500" />} title="Basic Information">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className={labelCls}>Event Name <span className="text-red-400">*</span></label>
                <input type="text" name="event_name" value={formData.event_name} onChange={handleChange} required className={inputCls} />
              </div>
              <div>
          ...
          {/* ── Schedule ── */}
          <div className="border-t border-slate-100" />
          <Section icon={<PencilLine className="w-3.5 h-3.5 text-indigo-500" />} title="Schedule">
          ...
          {/* ── Description ── */}
          <div className="border-t border-slate-100" />
          <Section icon={<PencilLine className="w-3.5 h-3.5 text-indigo-500" />} title="Event Description">
            <textarea
              name="description"
          ...
          <Section icon={<Bell className="w-3.5 h-3.5 text-indigo-500" />} title="Notification Settings">
            <p className="text-[11px] text-slate-400">You can selectively notify users about this event update.</p>
          ...
          <div className="flex justify-end pt-2 border-t border-slate-100">
            <button type="submit" disabled={isSubmitting}
              ...
              {isSubmitting ? (<><Loader2 className="w-4 h-4 animate-spin" /><span>Saving…</span></>) : (<><CheckCircle2 className="w-4 h-4" /><span>Save Changes</span></>)}
            </button>
          </div>
