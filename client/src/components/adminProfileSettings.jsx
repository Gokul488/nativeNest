  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        ...
        localStorage.setItem("user", JSON.stringify({ ...admin, account_type: "admin" }));
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        ...
        { name, email, mobile_number: mobileNumber },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <div className="max-w-2xl w-full bg-white shadow-lg rounded-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              className="bg-indigo-600 text-white px-8 py-3 rounded-lg hover:bg-indigo-700 transition duration-200 font-medium"
            >
              Update Profile
            </button>
          </div>
