...
// stallTypeController.js - Updated createStallType
const createStallType = async (req, res) => {
  ...
  try {
    ...
    // This creates the actual rows that builders will later "claim"
    for (let i = 1; i <= requestedAmount; i++) {
      await connection.query(
        'INSERT INTO stall (stall_number, event_id, stall_type_id, builder_id) VALUES (?, ?, ?, NULL)',
        [i, eventId, stallTypeId]
      );
    }

    await connection.commit();
    res.status(201).json({ message: 'Stall type created and stalls initialized' });
  } catch (err) {
    await connection.rollback();
    console.error(err);
    res.status(500).json({ error: 'Server error' });
